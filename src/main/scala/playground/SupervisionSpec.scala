...
class SupervisionSpec extends TestKit(ActorSystem("SupervisionSpec"))
  ...
  "A supervisor" should {
    "resume its child in case of a minor fault" in {
      ...
      child ! "Akka is awesome I am learning to think in a whole new way"
      child ! Report
      expectMsg(3)
    }
    ...
    "escalate on error when it doesn't know what to do" in {
  ...
  "A kinder supervisor" should {
    "not kill children in case it's restarted or escalates failures" in {
      val kinderSupervisor = system.actorOf(Props[NoDeathOnRestartSupervisor], "kinderSupervisor")
